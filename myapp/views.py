...
from django.http import JsonResponse
import json
from .models import Roster, Trailer

@csrf_exempt
def update_trailer_rego(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            roster_id = data.get('rosterId')
            trailer_number = int(data.get('trailerNumber'))
            trailer_rego = data.get('trailerRego')
            trailer_id = data.get('trailerId')

            # Fetch the roster record
            roster = Roster.objects.get(id=roster_id)

            # Fetch or create the trailer
            trailer, _ = Trailer.objects.get_or_create(id=trailer_id, defaults={'rego_number': trailer_rego})

            # Update the appropriate trailer field
            if trailer_number == 1:
                roster.trailer1 = trailer
            elif trailer_number == 2:
                roster.trailer2 = trailer
            elif trailer_number == 3:
                roster.trailer3 = trailer
            else:
                return JsonResponse({'status': 'error', 'message': 'Invalid trailer number.'})

            roster.save()
            return JsonResponse({'status': 'success'})
        except Roster.DoesNotExist:
            return JsonResponse({'status': 'error', 'message': 'Roster not found.'})
        except Exception as e:
            print(f"Error updating trailer rego: {e}")
            return JsonResponse({'status': 'error', 'message': str(e)})
    else:
        return JsonResponse({'status': 'error', 'message': 'Invalid request method.'})
